...
| Typecheck (strict, `tsc`) | harness-level `tsc` build | ✓ | non-zero | **FAIL** | 23 errors, all in `src/twins/dataCenter/omniverseAdapter.ts` (lines 214–380). Adapter constructs `Generator`, `CoolingUnit`, `CoolingZoneDetail`, `NetworkSwitch`, `NetworkPort`, `NetworkFabric`, `Firewall`, `EnvironmentalZone`, `SafetySensor`, and `Suppression` objects with fields that do not match `types/dataCenterTwin.ts`. Confirms audit finding: the Omniverse adapter is scaffolding that has drifted from its own type contracts. | **Yes** — reproduced without any source edit; `tsgo` did not catch it | `src/twins/dataCenter/omniverseAdapter.ts` |
