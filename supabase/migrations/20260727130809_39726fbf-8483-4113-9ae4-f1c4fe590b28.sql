-- Fix 1: dsx_current_state must obey caller RLS.
DROP VIEW IF EXISTS public.dsx_current_state;
CREATE VIEW public.dsx_current_state
...
-- Fix 2: digest() lives in pgcrypto (installed in public), not pg_catalog.
-- Recreate the RPC with a corrected search_path that resolves digest, and a
-- fully qualified reference for clarity.
...
DECLARE
  ...
  v_subject_hash text;
BEGIN
  v_subject_hash := encode(public.digest(coalesce(p_source_subject,''), 'sha256'), 'hex');
