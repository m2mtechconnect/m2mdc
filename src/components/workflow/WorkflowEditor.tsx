...
import { Canvas as FabricCanvas, Rect, Circle, Text, Line, FabricObject } from "fabric";
...
export function WorkflowEditor({ systemId, workflowId }: WorkflowEditorProps) {
  ...
  useEffect(() => {
    ...
    const canvas = new FabricCanvas(canvasRef.current, {
      width: canvasWidth,
      height: canvasHeight,
      backgroundColor: "hsl(0, 0%, 12%)", // --graphite-gray
      selection: true,
    });

    // Grid background with better visibility
    const gridSize = 24;
    const gridColor = 'hsl(0, 0%, 18%)'; // Slightly lighter for better visibility
  ...
  const createNodeObject = (type: string, x: number, y: number, nodeId: string): FabricObject => {
    const colors: Record<string, string> = {
      analyze: "hsl(227, 100%, 65%)", // electric-blue
      classify: "hsl(250, 75%, 60%)", // purple
      notify_teams: "hsl(51, 100%, 50%)", // signal-gold
  ...
  const handleSave = async () => {
    if (!currentWorkflowId && !systemId) {
      toast({
        title: "Cannot save",
        description: "No workflow or system ID provided",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (!wfId) {
        ...
        if (userError || !user) {
          throw new Error('Authentication required to save workflow');
        }
  ...
  const handleValidate = async () => {
    ...
    nodes.forEach(node => {
      if (node.type === 'analyze' && !node.config?.model) {
        errors.push(`Analyze node missing model configuration`);
      }
      if (node.type === 'classify' && (!node.config?.labels || node.config.labels.length === 0)) {
        errors.push(`Classify node missing classification labels`);
      }
    });
  ...
  const handleTestRun = async () => {
    ...
    try {
      ...
      toast({
        title: "Test run failed",
        description: error.message || "Failed to execute test run",
        variant: "destructive",
      });
    } finally {
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              disabled={!currentWorkflowId || isTestRunning || isSaving}
              className="gap-2"
            >
              {isTestRunning ? (
                <>
            ...
            <div className="pointer-events-auto rounded-xl border-2 border-dashed border-muted-foreground/30 bg-background/80 backdrop-blur-sm p-8 text-center max-w-md">
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                ...
                <Button 
                  size="sm" 
                  variant="outline"
                  onClick={() => handleAddNode('classify')}
                  className="gap-2"
                >
