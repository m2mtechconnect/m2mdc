...
window.addEventListener('keydown', e => { if (e.key === 'Escape') window.__win++; }, false);`);
  await drawer.press('Escape');
  await page.waitForTimeout(1200);
  console.log('DIAG cap/bub/win=', await page.evaluate('[window.__cap, window.__bub, window.__win, window.__stopped]'), 'state=', await drawer.getAttribute('data-state'));
