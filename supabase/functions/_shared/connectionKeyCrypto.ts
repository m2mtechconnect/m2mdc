/**
 * AES-GCM envelope for per-user managed connection handles.
 * Server-only. The plaintext handle never leaves an edge function.
 */
async function key(): Promise<CryptoKey> {
  const raw = Deno.env.get('APP_USER_CONNECTION_KEY_SECRET');
  if (!raw) throw new Error('managed_user_key_secret_unavailable');
