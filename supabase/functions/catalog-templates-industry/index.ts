/**
 * /v1/catalog-templates-industry
 * 
 * PURPOSE: List industry-specific agent templates
...
serve(createHandler({
  name: "catalog-templates-industry",
  authLevel: "public",
  ...
  handler: async (input, context) => {
    ...
    log("Fetching industry templates", { q, industry, tag, sort, page });

    let query = supabase
      .from('vw_templates_industry')
      .select('id, name, description, industry, tags, roi_pct, rating, downloads, certified, hero_icon, thumbnail_url, sample_prompts, kpi_definitions, default_config', { count: 'exact' });
