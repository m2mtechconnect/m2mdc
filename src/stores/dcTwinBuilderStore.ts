...
import { migrateTwinBuilderState, isTwinNameBroken } from '@/lib/utils/twinNameMigration';
  ...
  persist(
    (set, get) => ({
      ...
      initializeFromRecommendation: (recommendation, sessionId) => {
        ...
        // Use centralized company identity extraction
        const siteMeta: ScrapedSiteMeta = recommendation.siteMeta || {
          url: recommendation.url,
          domain: (() => {
            try {
              return new URL(recommendation.url).hostname.replace(/^www\./i, '');
            } catch {
        ...
        // If recommendation already has company name and twin name, use them; otherwise resolve
        let twinName = recommendation.twinName;
        let companyName = recommendation.companyName;
        
        if (!companyName || isTwinNameBroken(twinName)) {
          const companyIdentity = resolveCompanyIdentity(siteMeta);
          companyName = companyIdentity.companyName;
          twinName = generateCanonicalTwinName(companyIdentity);
        }
      ...
      initializeFromGreenDcRecommendation: (recommendation, sessionId) => {
        ...
        // Use centralized company identity extraction
        const siteMeta: ScrapedSiteMeta = {
          url: recommendation.domain ? `https://${recommendation.domain}` : '',
          domain: recommendation.domain || '',
          metaOgSiteName: recommendation.companyName,
        };
        
        const companyIdentity = resolveCompanyIdentity(siteMeta);
        const normalizedCompanyName = companyIdentity.companyName;
      ...
      loadFromStorage: () => {
        ...
        // After loading, ensure required entities exist and migrate broken names
        const currentState = get();
        let ensuredState = ensureRequiredEntities(currentState);
        
        // Migrate broken twin names if needed
        if (isTwinNameBroken(ensuredState.overview.twinName)) {
          const migrationResult = migrateTwinBuilderState(ensuredState);
          if (migrationResult.migrated) {
            console.log('[DCTwinBuilder] Migrated broken twin name:', migrationResult.originalName, '->', migrationResult.newName);
            ensuredState = {
              ...ensuredState,
              overview: {
                ...ensuredState.overview,
                twinName: migrationResult.newName,
                customerName: migrationResult.newCustomerName || ensuredState.overview.customerName,
              },
              isDirty: true,
            };
          }
