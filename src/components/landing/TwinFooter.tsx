...
export function TwinFooter() {
  ...
  const renderLink = (link: { label: string; href: string; external?: boolean; internal?: boolean }) => {
    if (link.internal) {
      return (
        <Link to={link.href} className="text-sm text-slate-400 hover:text-accent transition-colors">
          {link.label}
        </Link>
      );
    }
    if (link.external) {
      return (
        <a href={link.href} target="_blank" rel="noopener noreferrer" className="text-sm text-slate-400 hover:text-accent transition-colors inline-flex items-center gap-1">
          {link.label}
          <ExternalLink className="h-3 w-3 opacity-50" />
        </a>
      );
    }
    return <a href={link.href} className="text-sm text-slate-400 hover:text-accent transition-colors">{link.label}</a>;
  };
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-16">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8 lg:gap-12">
          ...
          <div className="col-span-2 md:col-span-4 lg:col-span-1 mb-8 lg:mb-0">
            ...
            <p className="text-sm text-slate-400 mb-6 max-w-xs">{t('landing.footerDescription')}</p>
      ...
      <div className="border-t border-slate-800">
        <div className="max-w-7xl mx-auto px-4 lg:px-8 py-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-sm text-slate-500">{t('landing.copyright', { year: currentYear })}</p>
