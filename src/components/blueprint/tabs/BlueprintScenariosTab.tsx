/**
 * Blueprint Scenarios Tab - Simulation scenarios with chain simulator
 */
...
export function BlueprintScenariosTab({ scenarios }: BlueprintScenariosTabProps) {
  ...
  const handleRunChain = (scenarioIds: string[]) => {
    navigate(`/data-centre-twin?view=simulation&chain=${scenarioIds.join(',')}`);
  };
                        ...
                        <div className="flex items-center gap-1">
                          {impact.direction === 'increase' ? (
                            <>
                              ...
                              <span className="text-success">+{impact.magnitude}%</span>
                            </>
                          ) : (
                            <>
                              <TrendingDown className="h-3 w-3 text-destructive" />
                              <span className="text-destructive">-{impact.magnitude}%</span>
                            </>
                          )}
                        </div>
                      </div>
